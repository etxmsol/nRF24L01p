...
void RF24::begin(void)
{
  ...
  write_register(CONFIG, 0x0f & ~_BV(PWR_UP)); 	// disable power up
  write_register(SETUP_AW, 0x03); 				// setup add width 5 bytes
  ...
  write_register(RX_PW_P0,32); //Number of bytes in data P1
  write_register(RX_PW_P1,32); //Number of bytes in data P1
  write_register(EN_RXADDR, 0x03); //Enable data P1
  write_register(TX_ADDR, m_txAddr, ADR_WIDTH); // write address into tx_add
  write_register(RX_ADDR_P0, m_txAddr, ADR_WIDTH); // write address into rx_add_p0
  write_register(RX_ADDR_P1, m_rxAddr, ADR_WIDTH); // write address into rx_add_p0
  write_register(EN_AA, 0x3f);     //enable auto-ack for all pipes
...
RF24::RF24(unsigned const char * txAddr, unsigned const char * rxAddr):
  wide_band(true), p_variant(false),
  payload_size(32), ack_payload_available(false), dynamic_payloads_enabled(false),
  pipe0_reading_address(0),
  m_txAddr(txAddr), 			// ptr to a 5 byte long array with TX pipe address
  m_rxAddr(rxAddr)				// ptr to a 5 byte long array with RX pipe address
{
}
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csnLow();
  spiTransfer( W_ACK_PAYLOAD | ( pipe & 0x7 ) );
  const uint8_t max_payload_size = 32;
